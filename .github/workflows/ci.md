...
`ci__deleted` filter is for — a `deleted:` status predicate over `.github/workflows/**` and
`.github/actions/**`, so it fires on the deletion itself, even beside an edit whose own claim
would otherwise mask it (from matched keys alone, deletion-plus-edit looks exactly like the edit).
The `ci__any` catch-all backs it up for a CI file that still exists but carries no key. Either way
the whole workspace validates instead of passing as "nothing changed" — coarse on purpose: it
knows a file went, not which one.

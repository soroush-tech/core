...
| 7   | Deploy to Cloudflare | `cloudflare/wrangler-action@v4` runs `wrangler pages deploy --branch=main`; project name + output dir come from `apps/web/wrangler.jsonc` (`name` + `pages_build_output_dir`). `packageManager: pnpm` is set explicitly — the lockfile sits at the repo root, so the action's per-directory detection would fall back to npm, which cannot read `workspace:*` deps |

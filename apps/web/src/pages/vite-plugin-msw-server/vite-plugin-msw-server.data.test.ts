...
describe('vite-plugin-msw-server.data', () => {
  it('names the scoped package and describes it', () => {
    expect(hero.name).toBe('@soroush.tech/vite-plugin-msw-server')
  ...
  it('points the npm and source links at this package', () => {
    ...
    expect(hero.repoUrl).toContain('/packages/vite-plugin-msw-server')
  })
})
